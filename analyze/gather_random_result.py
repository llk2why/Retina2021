...
result_txts = glob.glob('../results/Random/001_JOINTPIXEL_on_MIT_a={:.4f}_b={:.4f}/*/*.txt'.format(a,b))
...
def gather_result():
    ...
    for cfa in cfa_order:
        ...
        results[cfa]["McM"]
    datasets = ["MIT","moire","McM","Kodak","MyImage"]
    psnrs = OrderedDict()
    ssims = OrderedDict()
    ...
    plt.savefig('random/retina_result_a={:.4f}_b={:.4f}_{}.png'.format(a,b,date),dpi=300)

    with open('random/retina_result_a={:.4f}_b={:.4f}_{}.txt'.format(a,b,date),'w') as f:
        f.write('{:<15}'.format(''))
        for dataset in datasets:
