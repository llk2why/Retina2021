...
class HierarchicalBlock(nn.Module):
    def __init__(self, resnet_level=2):
        ...
        stage2 = [ResidualBlock(size=64) for _ in range(resnet_level)]
    ...
    def forward(self,input):
        ...
        return stage3_left_output,stage3_right_output

class JointPixel_MultiPath(nn.Module):
    def __init__(self, resnet_level=2,fusion_degree=2):
        super(JointPixel_MultiPath, self).__init__()
        self.fusion_degree = fusion_degree
        self.hierarchical_blocks = [HierarchicalBlock() for _ in range(fusion_degree)]
        ...
        stage4 = [ResidualBlock(size=64) for _ in range(resnet_level)]
    ...
    def forward(self, input):
        input_list = [input[:,i:i+3] for _ in range(self.fusion_degree)]

        fusion_stage3_left_output = []
        fusion_stage3_right_output = []
        for i,input in enumerate(input_list):
            stage3_left_output,stage3_right_output = self.hierarchical_blocks[i](input)
